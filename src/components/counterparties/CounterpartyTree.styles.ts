...
export const TreeItemWrapper = styled.div<{ $level: number }>`
  /* Відступ для вкладеності: 24px на рівень */
  padding-left: ${(p) => p.$level * 24}px;

  /* Лінія вкладеності (опціонально, додає структури) */
  ${(p) =>
    p.$level > 0 &&
    css`
      position: relative;
...
export const TreeItem = styled.div<{
  ...
  padding: 0.5rem 0.5rem 0.5rem 0.2rem; /* Трохи менший лівий паддінг, бо там кнопка expand */
  border-radius: 6px;
  cursor: pointer;
  transition: all 0.2s;
  user-select: none;
...
export const LeftSide = styled.div`
  display: flex;
  align-items: center;
  gap: 0.2rem; /* Відступ між стрілкою та іконкою */
  flex: 1;
  min-width: 0;
`;
...
export const PartialCheckBar = styled.div`
  width: 8px;
  height: 2px;
  background: white;
  border-radius: 1px;
...
// ... (решта стилів: LeftSide, ExpandButton, Info, IconWrapper, Label, Badge, Actions, ActionBtn)
