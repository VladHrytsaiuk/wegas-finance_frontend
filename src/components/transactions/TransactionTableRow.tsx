    ...
    return (
      <Table.Row onClick={() => onClick(tx.id)}>
        ...
        <Table.Cell className="col-actions">
          ...
          {/* mobile */}
          <S.MobileActions ref={ref} onClick={(e) => e.stopPropagation()}>
            {/* Обертаємо ВСЕ мобільне меню в Modal */}
            <Modal>
              <S.MenuToggle onClick={() => setOpen((v) => !v)}>
                <HiEllipsisVertical size={20} />
              </S.MenuToggle>

              {open && (
                <S.MenuDropdown onClick={() => setOpen(false)}>
                  {" "}
                  {/* <--- ЗАКРИВАЄМО ПРИ БУДЬ-ЯКОМУ КЛІКУ */}
                  <S.MenuItemLink
                    to={`/transactions/${tx.id}/edit`}
                    state={{ background: location }}
                  >
